...
class Orders(db.Model):
    ...
    # 订单号
    oid = db.Column(db.String(32), unique=True)
    # 用户id 外键 关联用户表
    ...
    # 订单总金额
    total_money = db.Column(db.Numeric(7, 2))
    # 实际金额
    real_money = db.Column(db.Numeric(7, 2))
    # 支付方式
    # 0 在线支付/ 1 货到付款
...
class OrderItem(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    # 外键  关联order订单表
    oid = db.Column(db.String(32), db.ForeignKey(Orders.oid, ondelete="CASCADE"))
    # 外键  关联User用户表
    uid = db.Column(db.Integer, db.ForeignKey(User.uid, ondelete="CASCADE"))
    # 所购买的商品id
...
class OrderRefunds(db.Model):
    ...
    # 退款金额
    refund_money = db.Column(db.Numeric(7, 2))
    # 商家拒绝退款原因
